<!-- <h1 style="font-size: 4em; font-weight: bold; background: #ff0; text-align: center; margin: 10px 0; letter-spacing: 0;">Vendor Creation by D.M. Office</h1> -->
<?php include('common/header.php'); ?>
			
	<div class="home pb-5">
		<div class="container">
			<div class="row">						
				<div class="col-lg-10 m-auto" >
					<div class="widget-area-2 proclinic-box-shadow">
						<h3 class="widget-title">Vendor Creation by D.M. Office</h3>
						<form>
							<div class="form-row control-group">
                            	<div class="form-group col-lg-6">
									<label>Name</label>
									<input type="text" class="form-control" placeholder="Name">
								</div>									
								<div class="form-group col-lg-6">
									<label>Contact Person</label>
									<input type="text" placeholder="contact" class="form-control">
								</div>
							</div>
							<div class="form-row control-group">
								<div class="form-group col-lg-6">
									<label>Contact No.</label>
									<input type="text" placeholder="contact" class="form-control">
								</div>
								<div class="form-group col-lg-6">
									<label>Email Id</label>
									<input type="Email" placeholder="email" class="form-control">
								</div>
							</div>
							<div class="form-row control-group">
								<div class="form-group col-lg-6">
									<label>Vendor Address</label>
									<textarea class="form-control" id="textAreaExample1" rows="1"></textarea>
								</div>
								<div class="form-group col-lg-6">
									<label>User Id</label>
									<input type="text" placeholder="User Id" class="form-control">
								</div>
							</div>
							<div class="form-row control-group">
								<div class="form-group col-lg-6">
									<label>Password</label>
									<input type="password" placeholder="*****" class="form-control">
								</div>
								<div class="form-group col-lg-6">
									<label>Account No.</label>
									<input type="text" placeholder="Account No." class="form-control">
								</div>
							</div>
							<div class="form-row control-group">
								<div class="form-group col-lg-6">
									<label>Bank Name</label>
									<input type="text" placeholder="Account No." class="form-control">
								</div>
								<div class="form-group col-lg-6">
									<label>Branch Name</label>
									<input type="text" placeholder="Account No." class="form-control">
								</div>
							</div>
							<div class="form-row control-group">
								<div class="form-group col-lg-6">
									<label>IFSC Code</label>
									<input type="text" placeholder="Account No." class="form-control">
								</div> 
								<div class="form-group col-lg-6 text-right">
									<label></label>
									<input type="button" value="Submit" class="btn btn-info d-block ml-lg-auto">
								</div>
                           	</div>	
						</form>									
					</div>
				</div>
			</div>			
		</div>
	</div>
	<?php include('common/footer.php'); ?>
