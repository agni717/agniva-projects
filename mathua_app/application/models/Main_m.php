...
class Main_M extends MY_Model {
	...
	public function getAll_list_of_Active_Advertisement($applyid = NULL){
		...
		$this->db->where('advertisement_master.adv_activated', 1);
		if($applyid != NULL){
	...
	public function getAll_list_of_ActiveforLogin_Advertisement($applyid = NULL){
		$cur_time = date('Y-m-d H:i:s');
	...
	public function getAll_list_of_OLDData_for_PrevApplication($applyid = NULL){
		
	}
	
	public function check_mobile_existence_forRegistration($mobile, $apply){
		$this->db->select('*');
		$this->db->from('frontend_users');
		$this->db->where('f_mobile', $mobile);
		$this->db->where('f_applied_for', $apply);
	...
	public function check_mobile_existence_forRegistration_confirm($mobile, $apply){
		$this->db->select('*');
		$this->db->from('frontend_users');
		$this->db->where('f_mobile', $mobile);
		$this->db->where('f_applied_for', $apply);
		$this->db->where('f_registered', 1);
		$this->db->where('f_status', 1);
		$query = $this->db->get();
		if($query->num_rows() > 0)
		{
	...
	public function check_email_existence_forRegistration_confirm($email, $apply){
		$this->db->select('*');
		$this->db->from('frontend_users');
		$this->db->where('f_email', $email);
		$this->db->where('f_applied_for', $apply);
		$this->db->where('f_registered', 1);
		$this->db->where('f_status', 1);
		$query = $this->db->get();
	...
	public function checkEmail_Register_member($email, $applyfor) {
        $sql = $this->db->select('*')->from('frontend_users')->where('f_email', $email)->where('f_applied_for', $applyfor)->where('f_registered', 1)->where('f_status', 1);
        $res = $this->db->get();
	...
	public function checkRegister_member($mobile, $applyfor) {
        $sql = $this->db->select('*')->from('frontend_users')->where('f_mobile', $mobile)->where('f_applied_for', $applyfor)->where('f_registered', 1)->where('f_status', 1);
        $res = $this->db->get();
	...
	public function insertRegistration_details_intheDB($rows, $f_uid = NULL){
		...
		if($f_uid != NULL){
			...
			if($this->db->insert("frontend_users", $rows)){
				...
				$row2_arr = array('fu_master'=>$user_id);
				$this->db->set($row2_arr);
				if($this->db->insert("f_user_details", $row2_arr)){
					return TRUE;
				}else{
					$this->db->delete('frontend_users', array('f_uid' => $user_id));
					return FALSE;
				}
			}else{
				return FALSE;
	...
	public function update_Candidate_user_log($rows) {
        $this->db->set($rows);
        $this->db->insert('front_user_log_tab',$rows);
    }
	

	public function check_mobile_existence_forUpdation($mobile, $apply){
		$this->db->select('*');
		$this->db->from('frontend_users');
		$this->db->where('f_mobile', $mobile);
		$this->db->where('f_applied_for', $apply);
	...
	public function check_Email_existence_forUpdation($email, $apply){
		$this->db->select('*');
		$this->db->from('frontend_users');
		$this->db->where('f_email', $email);
		$this->db->where('f_applied_for', $apply);
	...
	public function check_mobile_OTP_forUpdation($types, $old, $new, $otp, $uid){
		$curenttime = date('Y-m-d H:i:s');

		$this->db->select('*');
		$this->db->from('mobile_email_modify_tab');
		$this->db->where('mbe_type', $types);
		$this->db->where('mbe_mob_email', $old);
		$this->db->where('mbe_new_mob_email', $new);
		$this->db->where('mbe_otp', $otp);
		$this->db->where('mbe_id',$uid);
		$this->db->where('mbe_created_by', $this->session->userdata['member_id']);
		$this->db->where('mbe_otp_time <= "'.$curenttime.'"');
		$this->db->where('mbe_otp_time_end >= "'.$curenttime.'"');
		$query = $this->db->get();
		if($query->num_rows() > 0)
		{
			return TRUE;
	...
	public function storeSet_MobileUpdation_Candidate($rows, $sectionid = NULL){
		
		$this->db->set($rows);
		if($sectionid != NULL){
			$this->db->where('mbe_id', $sectionid);
			if($this->db->update("mobile_email_modify_tab", $rows)){
				return TRUE;
			}else{
				return FALSE;
			}
		}else{
			if($this->db->insert("mobile_email_modify_tab", $rows)){
				return TRUE;
			}else{
				return FALSE;
			}
	...
	public function getLastRow_ofinsertionin_Table($types, $old, $new, $otp){
		$this->db->select('*');
		$this->db->from('mobile_email_modify_tab');
		$this->db->where('mbe_type', $types);
		$this->db->where('mbe_mob_email', $old);
		...
		$this->db->where('mbe_otp', $otp);
		$this->db->where('mbe_created_by', $this->session->userdata['member_id']);
		//$this->db->where('mbe_status', 1);
		$this->db->order_by('mbe_id','DESC');
		$this->db->limit(1, 0);
		$query = $this->db->get();
		return $query->row();
	}

	/*((`event_start_datetime` >= '".$start_time."' AND `event_start_datetime` < '".$end_time."')
        OR (`event_end_datetime` > '".$start_time."' AND `event_end_datetime` <= '".$end_time."') OR
        (`event_start_datetime` <= '".$start_time."' AND `event_end_datetime` >= '".$end_time."'))*/
	...
	public function insertUpdate_Payment_candidateSet($rows, $tr_no = NULL){
		$this->db->set($rows);
		if($tr_no != NULL){
			$this->db->where('fu_transaction_no', $tr_no);
			if($this->db->update("f_user_transaction_tab", $rows)){
				return TRUE;
			}else{
				return FALSE;
			}
		}else{
			if($this->db->insert("f_user_transaction_tab", $rows)){
				return TRUE;
			}else{
				return FALSE;
			}
	...
	public function updateSuccess_Payment_candidateSet($rows, $tr_no, $cand_id){
		$this->db->trans_start();
		
		$this->db->set($rows);
		$this->db->where('fu_transaction_no', $tr_no);
		$this->db->update("f_user_transaction_tab", $rows);
		
		$row1 = array('fu_payment_stat'=>1,'fu_payment_date'=>date('Y-m-d H:i:s'));
		$this->db->set($row1);
        $this->db->where('fu_master', $cand_id);
        $this->db->update("f_user_details",$row1);
