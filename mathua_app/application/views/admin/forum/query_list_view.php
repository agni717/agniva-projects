              ...
              <div class="box box-warning">
                ...
                <div class="box-body">
                  ...
                  <table class="table table-striped" id="datatable_tab" width="100%">
                  	...
                  	<tbody>
                  		<?php foreach($q_list as $keys=>$quaries)
                  		{ ?>
                  		<tr>
                  			<td><?php echo $keys+1; ?></td>
                  			<td><?php echo $quaries->query_no; ?></td>
                  			<td><?php echo $quaries->query_subject; ?></td>
                  			<td><?php echo $quaries->f_full_name; ?></td>
                  			<td><?php echo date('d-m-Y',strtotime($quaries->query_createdate)); ?></td>
							<td><?php if($quaries->query_is_reply == 0){ echo '<span style="color:red">Not Given Yet</span>';}else{echo '<span style="color:green">Replied</span>';
							if($quaries->query_publish == 0){ echo '<span style="color:blue"> (Not Publish)</span>';}else{echo '<span style="color:blue"> (Published)</span>';}
							} ?></td>
							<?php if($this->session->userdata['utype'] <= 2){ ?>
                  			<td>
                  				<a onclick="goto_details_view('<?php echo $quaries->query_no; ?>');" href="javascript:;" title="View Record"><i class="fa fa-eye text-warning"></i></a>&nbsp;
                  				<?php if($quaries->query_is_reply == 0){ ?>
									<a onclick="goto_reply_view('<?php echo $quaries->query_no; ?>');" href="javascript:;" title="Reply Record"><i class="fa fa-reply text-warning"></i></a>&nbsp;
									<a onclick="return confirm('You are about to delete a record. This cannot be undone. Are you sure?');" href="<?php echo base_url().'admincontrol/panel/delete_work/'.$quaries->query_no; ?>" title="Delete Record"><i class="fa fa-trash-o text-warning"></i></a>&nbsp;
								<?php }else{
										if($quaries->query_publish == 0){ ?>
										<a href="<?php echo base_url().'admincontrol/discussion/publish_toall/'.$quaries->query_no; ?>" title="Publish Record"><i class="fa fa-lock text-warning"></i></a>&nbsp;
										<?php }else{ ?>
										<a href="<?php echo base_url().'admincontrol/discussion/hide_toall/'.$quaries->query_no; ?>" title="Hide Record"><i class="fa fa-unlock text-warning"></i></a>
										<?php }
									} ?>
                  			</td>
							<?php } ?>
