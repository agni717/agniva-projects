...
<!-- Content Wrapper. Contains page content -->
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
		  		Generate Final Panel List
          </h1>
          <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Generate Final Panel List</li>
          </ol>
        </section>
				...
				<div class="col-sm-3">
				  <div class="form-group">
					<label>Category</label>
					  <select class="form-control selectpicker" name="advcat_name" id="advcat_name" autocomplete="off" <?php if(empty($searchlist['advcat_name'])){echo 'disabled';} ?> onchange="goto_set_vacancy();">
						<option value="">---Select---</option>
				...
				<div class="col-sm-3">
				  <div class="form-group">
					<label>Generate For</label>
					  <select class="form-control selectpicker" name="gen_set" id="gen_set" autocomplete="off" onchange="goto_checkall_vacancy();">
						<option value="">---Select---</option>
				...
				<div class="col-sm-offset-4 col-sm-4">
				  <div class="form-group">
					<label>Total Vacancy</label>
					  <input class="form-control" name="vac_total" id="vac_total" value="<?php echo set_value('vac_total'); ?>" autocomplete="off">
					  <small class="text-error vac_total"><?php echo form_error('vac_total'); ?></small>
				  </div>
				</div>
				<div class="clearfix"></div>
				<div class="col-sm-12">
...
<script type="text/javascript">
	...
	function goto_set_vacancy(){
		$("#gen_set").val('');
	}

	function goto_checkall_vacancy(){
		var delay = 8000;
		var e_error = 0;
		...
		if(rf_set == ""){
			e_error = 1;
			error_message = error_message + '<br/>Recruitment For is Required.';
		}
		
		if(advno == ""){
			e_error = 1;
			error_message = error_message + '<br/>Advertisement No. is Required.';	
		}

		if (advcat_name == "") {
			e_error = 1;
			error_message = error_message + '<br/>Advertisement Category is Required.';	
		}

		if(gen_set == ""){
			e_error = 1;
			error_message = error_message + '<br/>Generate is Required.';
		}
		...
		if (e_error == 1) {
			...
			form_data.append('gen_set', gen_set);
			$.ajax({
				method: 'POST',
				url: '<?php echo base_url() . "admincontrol/finalpanel/getvacancy_from_advertisement"; ?>',
				data: form_data,
				dataType: 'JSON',
				contentType: false,
				processData: false,
	...
	function goto_submit_button(){
		...
		var advcat_name = $('#advcat_name option:selected').val();
		var gen_set = $("#gen_set option:selected").val();
		var vac_total = $("#vac_total").val();
		//alert(vac_total);return;
		...
		if(vac_total == ""){
			e_error = 1;
			$('.vac_total').html('Total Vacancy is Required.');
		}else{
			if(!vac_total.match(onlynumerics)){
				e_error = 1;
				$('.vac_total').html('Total Vacancy for only use Numeric Values, Check again.');
			}else if(vac_total <= 0){
				e_error = 1;
				$('.vac_total').html('Total Vacancy always greater than 0, Check again.');
			}else{
				$('.vac_total').html('');
			}	
		}
