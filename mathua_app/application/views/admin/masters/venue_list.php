				...
				<!-- <div class="box-footer clearfix no-border">
                  ...
                  <table class="table table-striped" id="datatable_tab" width="100%">
	                  <thead style="font-weight: bold;">
	                  		<td>Sl No.</td>
	                  		<td>Venue Name</td>
							<!--<td>Total Table No.</td>-->
							<td>District</td>
							<td>Status</td>
	                  		<td>Action</td>
	                  </thead>
                  	<tbody>
                  		<?php foreach($vdiv_list as $keys=>$users)
                  		{ ?>
                  		<tr>
                  			<td><?php echo ($keys + 1); ?></td>
                  			<td><?php echo $users->address_name; ?></td>
							<!--<td><?php //echo $users->address_tableno; ?></td>-->
							<td><?php echo $users->district_name; ?></td>
							<td><?php if($users->address_status == 1){ ?>
								  <span style="color:green;">Active</span>
							  <?php }elseif($users->address_status == 0){ ?>
								<span style="color:red;">InActive</span>
							  <?php } ?></td>
                  			<td>
                  				<a href="<?php echo base_url().'admincontrol/masterdata/modify_venueset/'.$users->address_id; ?>" title="Edit Record"><i class="fa fa-edit text-warning"></i></a>
                  				<?php if($users->address_status == 1){ ?>	
                  				<a href="<?php echo base_url().'admincontrol/masterdata/lock_venueset/'.$users->address_id; ?>" title="Lock Record"><i class="fa fa-unlock text-warning"></i></a>
                  				<?php } else { ?>
								<a href="<?php echo base_url().'admincontrol/masterdata/unlock_venueset/'.$users->address_id; ?>" title="Unock Record"><i class="fa fa-lock text-warning"></i></a>
								<?php } ?>
