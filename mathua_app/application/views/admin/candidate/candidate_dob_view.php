              ...
              <div class="box box-warning">
                ...
                <div class="box-body">
					<div class="row">
						<div class="col-sm-12 text-center">
							...
							Advertisement No. :- <?php echo $fuser_detailset->adv_no; ?><br/><br/>
						Current DOB - <?php echo date('d-M-Y',strtotime($fuser_detailset->fu_dob)); ?></p>	
						</div>
						<div class="col-sm-12">
				...
				<div class="col-sm-offset-3 col-sm-6">
				  <div class="form-group">
					<label>Candiate Date of Birth for Modifcation</label>
					  <input class="form-control" name="age_dob" id="age_dob" value="<?php echo date('d-m-Y',strtotime($fuser_detailset->fu_dob)); ?>" placeholder="" autocomplete="off" />
				      <small class="text-error age_dob"><?php echo form_error('age_dob'); ?></small>
				  </div>
				</div>
...
<script src="<?php echo base_url().'bootstrap-admin/plugins/datatables/dataTables.bootstrap.js'; ?>" type="text/javascript"></script>
	<script type="text/javascript">
		...
		function goto_submit_button(){
			...
			if (age_dob == "") {
				e_error = 1;
				$('.age_dob').html('Candidate DOB is Required.');
			} else {
				if(isDatecheck(age_dob) == false){
					e_error = 1;
					$('.age_dob').html('Candidate DOB Format check properly and Try Again.');
				} else {
					$('.age_dob').html('');
				}
			}
			...
			if(e_error == 1){
				...
				//alert(rehash);
				var conf_answer = confirm("Are you sure you want to modify Candidate Date of Birth?")
				if (conf_answer){
					var form_data = new FormData();
					form_data.append('main_refid', main_refid);
					form_data.append('age_dob', age_dob);
					$.ajax({
						method: 'POST',
						url: '<?php echo base_url() . "admincontrol/candidates/cand_dobset_submission"; ?>',
						data: form_data,
						dataType: 'JSON',
						contentType: false,
						processData: false,
						success: function(data) {
							...
							if (data.msg == 1) {
								...
								$('.div_roller_total').fadeOut();
								$('.get_success_total').html('Candidate DOB is Updated Successfully.');
