                ...
                <div class="box-body">
					<div class="row">
						<div class="col-sm-12 text-center">
							...
							<p style="font-size:18px;font-weight:bold;">Current Status - 
							<?php if($detail_result->cr_approval == "Rejected"){
								echo '<span style="color:red">'.$detail_result->cr_approval.'</span><br/>Reason of Rejection :- '.$detail_result->cr_reject_comments.' ';
								if(!empty($detail_result->cr_reject_document)){
									echo '<a href="'.$urllink.$detail_result->cr_reject_document.'" target="_blank">(Attach Document)</a>';
								}
							}elseif($detail_result->cr_approval == "Approved"){
								echo '<span style="color:green">'.$detail_result->cr_approval.'</span>';
							}elseif($detail_result->cr_approval == "NotChecked"){
								echo '<span style="color:blue">'.$detail_result->cr_approval.'</span>';
							} ?>
							</p>
						</div>
						<div class="col-sm-12">
						<?php if(!empty($rejection_list)){ ?>
						<div align="center"><a href="<?php echo base_url().'admincontrol/dashboard/candidates_checkingdetails_printsets/'.$fuser_detailset->f_application_no; ?>" target="_blank" class="btn btn-lg btn-primary">Print</a></div>
						<?php } ?>
						<strong style="font-size:20px;">Checking Details :-</strong><br/>
						<?php $str_arr = array(
							//'f_mobile' => 'Mobile',
							//'f_email' => 'Email-ID',
								...
								if($reject_items->chk_type == "fu_age_relax"){
									...
									foreach($allquali_list as $quali_items){
										if($reject_items->chk_sub_typeid == $quali_items->aquali_exam){
											echo "<p><strong>".($keys+1).". ".$str_arr[$reject_items->chk_type]." || ".$quali_items->qm_name."</strong> - ".$reject_items->chk2_approve." by the HRB Administrator (Final Checked by ".$reject_items->firstname." ".$reject_items->lastname." on ".date('d/m/Y h:i A',strtotime($reject_items->chk2_appro_date)).")<br/><strong>Detail Reason :</strong> ".$reject_items->chk2_comments."</p><hr/>";
											break;
										}
									}
									...
									foreach($allexp_list as $exp_items){
										if($reject_items->chk_sub_typeid == $exp_items->aexpr_name){
											echo "<p><strong>".($keys+1).". ".$str_arr[$reject_items->chk_type]." || ".$exp_items->expset_name."</strong> - ".$reject_items->chk2_approve." by the HRB Administrator (Final Checked by ".$reject_items->firstname." ".$reject_items->lastname." on ".date('d/m/Y h:i A',strtotime($reject_items->chk2_appro_date)).")<br/><strong>Detail Reason :</strong> ".$reject_items->chk2_comments."</p><hr/>";
											break;
										}
									}
								}else{
									echo "<p><strong>".($keys+1).". ".$str_arr[$reject_items->chk_type]."</strong> - ".$reject_items->chk2_approve." by the HRB Administrator (Final Checked by ".$reject_items->firstname." ".$reject_items->lastname." on ".date('d/m/Y h:i A',strtotime($reject_items->chk2_appro_date)).")<br/>
									<strong>Detail Reason :</strong> ".$reject_items->chk2_comments."</p><hr/>";
								}
							} 
						}else{ 
							echo "<p align=\"center\"><strong>Final Checking not Available.</strong></p><hr/>";
						}?>
						</div>
						<div class="col-sm-12">
							<strong style="font-size:20px;">Checkers Log :-</strong><br/>
							<table class="table table-bordered">
								...
								<tbody>
									<?php if(!empty($check_log_list)){
										$chklog_counter = 1;
										foreach($check_log_list as $keys=>$logitem){
											if(in_array($logitem->chklog_type, $str_arr2)){
												if($logitem->chklog_type == "fu_age_relax"){
													foreach($extraage_list as $extage_items){
														if($logitem->chklog_type_id == $extage_items->fu_ext_ageid){
															echo "<tr><td>".$chklog_counter."</td><td>".$str_arr[$logitem->chklog_type]." (".$extage_items->caste_name.")</td><td>".$logitem->chklog_approval."</td><td>".$logitem->firstname." ".$logitem->lastname."</td><td>".date('d/m/Y h:i A',strtotime($logitem->chklog_createdate))."</td><td>".$logitem->chklog_msg."</td></tr>";
															break;
														}
													}
												}elseif($logitem->chklog_type == "fu_es_qualification" || $logitem->chklog_type == "fu_ds_qualification"){
													foreach($allquali_list as $quali_items){
														if($logitem->chklog_type_id == $quali_items->aquali_exam){
															echo "<tr><td>".$chklog_counter."</td><td>".$str_arr[$logitem->chklog_type]." (".$quali_items->qm_name.")</td><td>".$logitem->chklog_approval."</td><td>".$logitem->firstname." ".$logitem->lastname."</td><td>".date('d/m/Y h:i A',strtotime($logitem->chklog_createdate))."</td><td>".$logitem->chklog_msg."</td></tr>";
															break;
														}
													}
												}elseif($logitem->chklog_type == "fu_has_es_service" || $logitem->chklog_type == "fu_has_ds_service"){
													foreach($allexp_list as $exp_items){
														if($logitem->chklog_type_id == $exp_items->aexpr_name){
															echo "<tr><td>".$chklog_counter."</td><td>".$str_arr[$logitem->chklog_type]." (".$exp_items->expset_name.")</td><td>".$logitem->chklog_approval."</td><td>".$logitem->firstname." ".$logitem->lastname."</td><td>".date('d/m/Y h:i A',strtotime($logitem->chklog_createdate))."</td><td>".$logitem->chklog_msg."</td></tr>";
															break;
														}
													}
												}else{
													echo "<tr><td>".$chklog_counter."</td><td>".$str_arr[$logitem->chklog_type]."</td><td>".$logitem->chklog_approval."</td><td>".$logitem->firstname." ".$logitem->lastname."</td><td>".date('d/m/Y h:i A',strtotime($logitem->chklog_createdate))."</td><td>".$logitem->chklog_msg."</td></tr>";
												}
												$chklog_counter++;
											}
										}
