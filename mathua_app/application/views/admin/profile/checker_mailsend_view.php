...
<!-- Content Wrapper. Contains page content -->
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            Checker's Mail Send Section
          </h1>
          <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Checker's Mail Send Section</li>
          </ol>
        </section>
          ...
          <div class="row">
		    ...
		    <?php $this->session->unset_userdata('e_error'); } ?>
              ...
              <div class="box box-warning">
				...
				<div class="col-sm-2">
					<div class="form-group bootstrap-timepicker">
					<label class="control-label">Closing Time<font style="color: red;">*</font></label>
                  ...
                  <table class="table table-striped" id="datatable_tab" width="100%">
	                  <thead style="font-weight: bold;">
							...
							<td>Candidate Mobile</td>
	                  		<td>Doc Type</td>
	                  		<td>Date-time</td>
							<td>Candidate Reply</td>
	                  </thead>
                  	<tbody>
                  		<?php
						$comname_arr = array('','Picture','Signature','Address Proof','Date of Birth Proof','Caste Proof','PWD Proof','Exempted Proof','Ex-Service Proof','EWS Proof','Registration Certificate');
						foreach($total_checkinglist as $keys=>$users)
                  		...
                  		<tr>
							...
							<td><?php 
							if($users->udm_doctype == 'CO'){
								echo $comname_arr[$users->udm_doc_id];
							}elseif($users->udm_doctype == 'EQ'){
								//$docu_details = $this->candidates_m->get_EssenQualification_fromDB($users->udm_doc_id);
								echo 'Essential Qualification Document</p>';
							}elseif($users->udm_doctype == 'DQ'){
								//$docu_details = $this->candidates_m->get_DesireQualification_fromDB($users->udm_doc_id);
								echo 'Desirable Qualification Document</p>';
							}elseif($users->udm_doctype == 'ES'){
								//$docu_details = $this->candidates_m->get_EssenExperience_fromDB($users->udm_doc_id);
								echo 'Essential Experience Document</p>';
							}elseif($users->udm_doctype == 'DS'){
								//$docu_details = $this->candidates_m->get_DesireExperience_fromDB($users->udm_doc_id);
								echo 'Desirable Experience Document</p>';
							}elseif($users->udm_doctype == 'EA'){
								//$docu_details = $this->candidates_m->get_EssenAgeRelax_fromDB($users->udm_doc_id);
								echo 'Age Relaxation Document</p>';
							}
							?></td>
							<td><?php echo date('d-m-Y h:i:s A',strtotime($users->udm_createdate)); ?></td>
							<td><?php if($users->udm_status == 1){
								echo "<span style='color:red;'>Not Replied</span>";
							}elseif($users->udm_status == 2){
...
<script type="text/javascript">
	...
	function goto_checker_search(){
		...
		if(rf_set != "" && advno != ""){
			...
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('admincontrol/dashboard/get_allcheckermail_against_advertisement') ?>",
				dataType: 'json',
				data: form_data,
				contentType:false,
				cache:true,
