				...
				<div class="col-sm-offset-3 col-sm-6">
				  <div class="form-group">
					<label>Reason of Rejection</label>
					  <textarea class="form-control" name="main_reason" id="main_reason" autocomplete="off"></textarea>
				      <small class="text-error main_reason"><?php echo form_error('main_reason'); ?></small>
				  </div>
				</div>
				...
				<div class="col-sm-offset-3 col-sm-6">
				  <div class="form-group">
					<label>Upload Document</label>
					  <input class="form-control" name="mainfile" id="mainfile" autocomplete="off" type="file" />
					  <small class="text-error mainfile"><?php echo form_error('mainfile'); ?></small>
				  </div>
				</div>
				<div class="col-sm-3">&nbsp;</div>
				<div class="col-sm-12">
...
<script src="<?php echo base_url().'bootstrap-admin/plugins/datatables/dataTables.bootstrap.js'; ?>" type="text/javascript"></script>
	<script type="text/javascript">
		...
		function goto_submit_button(){
			...
			var main_reason = $("#main_reason").val();
			var files = $('#mainfile')[0].files;
			...
			/*if(main_reason != ""){
				if(!main_reason.match(alphanumerics_no)){
					e_error = 1;
					$('.main_reason').html('Reason not use special charecters [without _ / : ( [ + ; % = < > ] @ \' & . ) , -], Check again.');
				}else{
					$('.main_reason').html('');
				}	
			}*/
			
			if (document.getElementById("mainfile").files.length == 0) {
				e_error = 1;
				$('.mainfile').html('Document is Required.');
			} else {
				var fileInput = document.getElementById('mainfile');
				var filePath = fileInput.value;
				if (!allowedExtensions.exec(filePath)) {
					e_error = 1;
					$('.mainfile').html('Document type Invalid.(Use PDF/JPG)');
				} else {
					$('.mainfile').html('');
				}
			}
			...
			if(e_error == 1){
				...
				//alert(rehash);
				var conf_answer = confirm("Are you sure you want to Reject the Candidate?")
				if (conf_answer){
					var form_data = new FormData();
					form_data.append('main_refid', main_refid);
					form_data.append('main_reason', main_reason);
					form_data.append("files", files[0]);
					$.ajax({
						method: 'POST',
						url: '<?php echo base_url() . "admincontrol/dashboard/cand_rejection_submission"; ?>',
						data: form_data,
						dataType: 'JSON',
						contentType: false,
						processData: false,
						success: function(data) {
							...
							if (data.msg == 1) {
								...
								$('.div_roller_total').fadeOut();
								$('.get_success_total').html('Candidate is Rejected Successfully.');
