...
<!-- Content Wrapper. Contains page content -->
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            Marks Entry Monitoring Datewise
          </h1>
          <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Marks Entry Monitoring Datewise</li>
          </ol>
        </section>
			  ...
			  <div class="box-body">
								<?php if(!empty($appli_list)){ ?>
									<div class="row text-center" style="font-size:20px;">
										<form action="<?php echo base_url('admincontrol/dashboard/marksentry_monitoring_pdf_sets'); ?>" target="_blank" method="post" id="form_1">
				  ...
				  <div class="table-responsive" id="psetsss">
				  	...
				  	<div align="center" style="width:100%;font-size:24px;">
					  <?php $g_total = $total_days = $avg_total = 0;
					  for($daycnt = 0;$daycnt<count($all_dates); $daycnt++){
						$daytotal = 0;
						foreach($appli_list as $keys=>$users){
							$daytotal = $daytotal + $users['t_days'][$daycnt];
						}
						if($daytotal > 0){
							$g_total = $g_total + $daytotal;
							$total_days++;
						}
						//echo date('d-m-Y',strtotime($all_dates[$daycnt]));
					  }
					  $avg_total = ceil($g_total / $total_days);
					 ?>
						Total Entry - <?php echo $g_total; ?><br/>
						Total Working Days - <?php echo $total_days; ?><br/>
						Average Entry - <?php echo $avg_total; ?>
					</div>
									
                  <table border="1" cellpadding="3" class="" id="datatable_tab11" width="100%">
	                  <thead style="font-weight: bold;">
					  		<td width="12%">Name (Level)</td>
	                  		<td width="4.8%">Total</td>
	                  		<td width="5.2%">Average</td>
                            <?php for($daycnt = 0;$daycnt<count($all_dates); $daycnt++){ ?>
	                  		<td width="4%"><?php echo date('d-m-Y',strtotime($all_dates[$daycnt])); ?></td>
                            <?php } ?>
							<td width="5%">Total Working Days</td>
	                  </thead>
                  	<tbody>
					  	<!--<tr>
							<td colspan="<?php //echo (count($all_dates) + 4); ?>"><hr style="border-color:#999;" /></td>
						</tr>-->
                  		...
                  		{  if($users['t_all'] > 0){
							...
							?>
                  		<tr>
                  			<td width="12%"><?php echo "<strong>".($entrycounter + 1).".</strong> ".$users['cheker_name']." | ".$users['type_name']; ?></td>
                  			<td width="4.8%"><?php echo $users['t_all']; ?></td>
                  			<td width="5.2%"><?php echo $users['t_avg']; ?></td>
                            <?php for($dayresult = 0;$dayresult<count($users['t_days']); $dayresult++){ ?>
	                  		<td width="4%"><?php echo $users['t_days'][$dayresult]; ?></td>
                            <?php } ?>
							<td width="5%"><?php echo $users['t_work']; ?></td>
                  		</tr>	
						<?php $entrycounter++;}} ?>
                  	</tbody>
                  </table>
