...
<div class="row ">
  ...
  <div class="form-group">
    ...
    <div class="form-group otpset" style="display:none;">
		<label class="text-uppercase">Mobile OTP</label>
		<input type="text" class="form-control" placeholder="Mobile OTP" id="otp_sign" name="otp_sign" class="form-control" autocomplete="off" />
...
<script type="text/javascript">
				...
				$.ajax({
					...
					success:function(data){
						...
						if(data.msg == 1)
						{
							//console.log(data);
							$('.div_roller_total').fadeOut();
							$('.get_success_total').html('Both OTP send to Your Registered Mobile No. and Email. E-OTP - ' + data.mailsms + ', M-OTP - ' + data.mobsms);
	...
	function gotoclclickbutton(){
		...
		if(otp_sign == ""){
			e_error = 1;
			$('.otp_sign').html('Mobile OTP is required.');
		}else{
			if(!otp_sign.match(onlynumerics)){
				e_error = 1;
				$('.otp_sign').html('Mobile OTP needs only Numeric digits.');
			}else{
				$('.otp_sign').html('');
			}
		}
