...
.alert-error, .text-error, .redclass {
        ...
        <div class="presentation-container">
        	<div class="container">
				...
				<h1 class="panel-title">Modify Mobile Number</h1>
...
<script type="text/javascript">
	...
	function gotootpcheck(){
		...
		if(oldmobile == ""){
			e_error = 1;
			$('.oldmobile').html('Current Mobile No. is required.');
		}else{
			if(!oldmobile.match(onlynumerics)){
				e_error = 1;
				$('.oldmobile').html('Current Mobile No. needs only 10 digit.');
			}else if(oldmobile.length != 10){
				e_error = 1;
				$('.oldmobile').html('Current Mobile No. needs only 10 digit.');
			}else{
				$('.oldmobile').html('');
		...
		if(newmobile == ""){
			e_error = 1;
			$('.newmobile').html('New Mobile No. is required.');
		}else{
			if(!newmobile.match(onlynumerics)){
				e_error = 1;
				$('.newmobile').html('New Mobile No. needs only 10 digit.');
		...
		if(e_error == 1){
			...
			$.ajax({
				method:'POST',
				url:'<?php echo base_url()."member/get_otp_formobile_change_candidates"; ?>',
				...
				success:function(data){
					...
					if(data.msg == 1)
					{
						//console.log(data);
						$('.div_roller_total').fadeOut();
						$('.get_success_total').html('OTP send to Your New Mobile Number');
	...
	function gotoclclickbutton(){
		...
		if(e_error == 1){
			...
			$.ajax({
				method:'POST',
				url:'<?php echo base_url()."member/mobileno_modifcation_candidates"; ?>',
				...
				success:function(data){
					...
					if(data.msg == 1)
					{
						//console.log(data);
						$('.div_roller_total').fadeOut();
						$('.get_success_total').html('Mobile Number is Updated Successfully.');
						$('input').prop('');
						$(".get_success_total").fadeIn();
						setTimeout(function(){
