  	...
  	<div class="container">
    	<div class="row">
			...
			<div class="panel panel-default">
				...
				<h1 class="panel-title">Testing List</h1>
				...
				<?php echo form_open_multipart('','class="form-horizontal" id="form123"'); ?>
					...
					<div  class="form-group">
						...
						<div class="col-md-4">
                          ...
                          <select class="form-control" name="s_block" id="s_block" readonly>
						...
						<?php }elseif($this->session->userdata('member_utype') == 4){ ?>
						<div class="col-md-3">
					      <label class="bmd-label-floating">Select Block</label>
                          <select class="form-control" name="s_block" id="s_block">
						    <option value="">--- Select ---</option>
                          	<?php foreach($m_block_list as $s_blocks){ ?>
								<option value="<?php echo $s_blocks->block_id; ?>" <?php if(!empty($datelist['sblock'])){ if($datelist['sblock'] == $s_blocks->block_id){echo 'selected="selected"';}} ?>><?php echo $s_blocks->block_name; ?></option>
							<?php } ?>
						  </select>	
                          <small class="text-error s_block"><?php echo form_error('s_block'); ?></small>				
						...
						<div class="col-md-3">
                          ...
                          <select class="form-control" name="s_muni" id="s_muni">
						    <option value="">--- Select ---</option>
                          	<option value="<?php echo $block_list; ?>" <?php if(!empty($datelist['smuni'])){ if($datelist['smuni'] == $block_list){echo 'selected="selected"';}} ?>><?php echo $block_list; ?></option>
						  </select>	
                          <small class="text-error s_muni"><?php echo form_error('s_muni'); ?></small>				
						</div>
						<?php }elseif($this->session->userdata('member_utype') == 1){ ?>
						<div class="col-md-3">
					      <label class="bmd-label-floating">Select Block</label>
                          <select class="form-control" name="s_block" id="s_block">
						    <option value="">--- Select ---</option>
			...
			<table id="d_table_show" class="table table-bordered table-striped">
				...
				<?php foreach($formlist as $keys=>$forms){ ?>
					<tr>
						...
						<?php if($this->session->userdata('member_utype') >= 4 || $this->session->userdata('member_utype') == 1){ ?>
						<td>
							<?php if(empty($forms->ca_detail_id)){ ?>
						<a class="btn btn-xs btn-danger" href="<?php echo base_url()."member/new_bdo_form_details_entryupdate/".$forms->collect_id; ?>" style="margin-bottom:5px;">Update Form</a>
						<?php }else{ echo '<span style="color:green">Record Checked</span>'; } ?>
						</td>
						<?php } ?>
