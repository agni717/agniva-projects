...
<div class="container mt-3 container_area">
  ...
  <div><h3>Registration No. - <?php echo $fuser_detailset->f_application_no; ?><br/>
            All 4 Steps are Completed on <?php echo date('d-m-Y h:i A',strtotime($fuser_detailset->fu_step4_submitdate)); ?><br/>
  <?php if($fuser_detailset->fu_cancel_stat == 1){ ?><span style="color:red;">(Application is Cancelled by Candidate)</span><?php } ?>
  <?php if($fuser_detailset->fu_payment_stat != 1){ ?>
    <div align="center" style="color:red;">Application Preview<br/>This preview does not prove your application process is complete.</div>
	<?php if($fuser_detailset->fu_final_submit == 1){ ?><div align="center">(Application Final Submission is Done <?php if(!empty($fuser_detailset->fu_finalsubmit_date)){echo "on ".date('d-m-Y h:i A',strtotime($fuser_detailset->fu_finalsubmit_date));} ?>)</div><?php }else{ ?><div align="center">(Application Final Submission is Pending)</div><?php } ?>
	<div align="center">(Payment Processing is Pending)</div>
  <?php }else{ ?>
  <?php if($fuser_detailset->fu_final_submit == 1){ ?><div align="center">(Application Final Submission is Done <?php if(!empty($fuser_detailset->fu_finalsubmit_date)){echo "on ".date('d-m-Y h:i A',strtotime($fuser_detailset->fu_finalsubmit_date));} ?>)</div><?php }else{ ?><div align="center">(Application Final Submission is Pending)</div><?php } ?>
	<div align="center">(Payment is Done)</div>
  <?php } ?>
  </h3></div>
<table width="100%" border="0">
  ...
  <tr>
    <td><label><strong>Date Of Birth :</strong> <?php echo date('d-F-Y',strtotime($fuser_detailset->fu_dob)); ?></label></td>
  ...
  <tr>
    <td colspan="2"><label><strong>Caste :</strong></label> <?php $castesets = '';
    foreach ($caste_tab as $caste) : ?>
      <?php if ($fuser_detailset->fu_caste_type == $caste->caste_id){ echo $caste->caste_name; $castesets = $caste->caste_cat;} ?>
    <?php endforeach; ?>
    ...
    <tr>
    <td colspan="2"><label><strong>Exempted Category :</strong></label> <?php echo $fuser_detailset->fu_exempted; ?><br>
    
      <table width="100%" border="1" cellpadding="5" cellspacing="5" >
        <tr>
          <td><label><strong>Description  :</strong><?php echo $fuser_detailset->fu_exc_reason; ?></label></td>
    ...
    <tr>
    <td colspan="2"><label><strong>Ex Serviceman Category :</strong></label> <?php echo $fuser_detailset->fu_exservice; ?><br>
      
      <table width="100%" border="1" cellpadding="5" cellspacing="5" >
        <tr>
          <td><label><strong>Description  :</strong><?php echo $fuser_detailset->fu_exs_reason; ?></label></td>
    ...
    <tr>
    <td colspan="2"><label><strong>Sports Man Category  :</strong></label> <?php echo $fuser_detailset->fu_ews; ?><br>
      <table width="100%" border="1" cellpadding="5" cellspacing="5" >
        <tr>
          <td><label><strong>Description  :</strong><?php echo $fuser_detailset->fu_ews_reason; ?></label></td>
...
<tr>
  ...
  <tr>
    <td colspan="<?php if($quali_persu_check == 1){echo "10";}else{echo "9";}?>"><strong>Essential Qualification</strong> </td>
    </tr>
  <tr>
    ...
    <?php if($quali_persu_check == 1){ ?>
      <td><strong>Final Qualification Pursuing</strong></td>
    <?php } ?>
  ...
  <tr>
    <td colspan="<?php if($quali_persu_check == 1){echo "10";}else{echo "9";}?>"><strong>Desirable Qualification</strong> </td>
    </tr>
  <tr>
    <td><strong>Examination Name</strong></td>
    <td><strong>Board/ Council/ University/ Journal</strong></td>
  ...
  <tr>
    <td colspan="<?php if($quali_persu_check == 1){echo "10";}else{echo "9";}?>">
...
<div class="mt-3 mb-5 text-center">
      ...
      <?php //if(strtotime($adv_detail->adv_end_time) > strtotime(date('Y-m-d H:i:s'))){ ?>
        ...
        <?php //echo form_close(); ?>
        <?php }else{ 
            if($adv_detail->adv_type == "New" && $fuser_detailset->fu_final_submit == 1 && $fuser_detailset->fu_cancel_stat != 1){ ?>
            <?php if($fuser_detailset->fu_pay_amount != 0.00 && $fuser_detailset->fu_payment_stat != 1){ ?>
...
<!--(strtotime($adv_detail->adv_end_time) > strtotime(date('Y-m-d H:i:s')))-->
<?php $cctime_set = strtotime(date('Y-m-d H:i:s'));
if((($adv_detail->adv_type == "New" && (strtotime($adv_detail->adv_end_time) > $cctime_set) && ($cctime_set > strtotime($adv_detail->adv_start_time))) || ($adv_detail->adv_type == "Old" && (strtotime($adv_detail->adv_old_endtime) > $cctime_set) && ($cctime_set > strtotime($adv_detail->adv_old_starttime)))) && (($fuser_detailset->fu_final_submit != 1 && $fuser_detailset->fu_cancel_stat != 1) || $fuser_detailset->fu_payment_stat == 0)){ ?>
<script type="text/javascript">
          ...
          $.ajax({
            ...
            success: function(data) {
              ...
              if (data.msg == 1)
              {
                ...
                //alert(data.msg[0].space_rate);
                $('.div_roller_total').fadeOut();
                $('.get_success_total').html('Final Submission is Done Successfully.');
                $(".get_success_total").fadeIn();
                $('input, select').val('');
                $('input').html('');
                setTimeout(function() {
                ...
                setTimeout(function() {
                  window.location.replace("<?php echo site_url('member/finalcheck_up') ?>");
                }, 3000);
