  	...
  	<div class="container">
    	<div class="row">
			...
			<div class="panel panel-default">
				...
				<h1 class="panel-title">Testing Report</h1>
        ...
        <div class="panel-body">
				...
				<?php echo form_open_multipart('','class="form-horizontal" id="form123"'); ?>
					...
					<div  class="form-group">
                        ...
                        <div class="col-md-3">
					      <label class="bmd-label-floating">Select Health District</label>
                          <select class="form-control" name="hd_set" id="hd_set">
                          <option value="">--- Select ---</option>
                          <option value="ALL" <?php if(!empty($datelist['hdset'])){ if($datelist['hdset'] == "ALL"){echo 'selected="selected"';}} ?>>ALL</option>
                          <?php foreach($hd_list as $key=>$lists){ ?>
                          <option value="<?php echo $lists->hd_dist; ?>" <?php if(!empty($datelist['hdset'])){ if($datelist['hdset'] == $lists->hd_dist){echo 'selected="selected"';}} ?>><?php echo $lists->hd_dist; ?></option>
                          <?php } ?>
                          </select>	
                          <small class="text-error hd_set"><?php echo form_error('hd_set'); ?></small>				
                        </div>
                        <div class="col-md-3">
			...
			<table id="d_table_show" class="table table-bordered table-striped">
				...
				<?php foreach($formlist as $keys=>$forms){ ?>
					<tr>
						...
						<td><a class="btn btn-xs btn-warning" href="<?php echo base_url()."member/new_bphc_form_update_submission/".$forms->collect_id; ?>" style="margin-bottom:5px;">Update Form</a>
						<a onclick="return confirm('You are about to Delete a record. This cannot be undone. Are you sure?');" href="<?php echo base_url('member/delete_record/'.$forms->collect_id); ?>" class="btn btn-xs btn-danger">Delete Record</a></td>
					</tr>
				<?php } ?>
				</tbody>
			</table>
...
<script>
	...
	function goto_submit_button(){
		...
		if(p_sdate != ""){
			if(isDatecheck(p_sdate) == false){
				e_error = 1;
				$('.p_sdate').html('Start Date Format check properly and Try Again.');
			}else{
				$('.p_sdate').html('');
			}	
		}
		...
		if(p_edate != ""){
			if(isDatecheck(p_edate) == false){
				e_error = 1;
				$('.p_edate').html('End Date Format check properly and Try Again.');
			}else{
				$('.p_edate').html('');
			}	
		}
		...
		}else if(p_sdate == "" && p_edate != ""){
			e_error = 1;
			error_message = 'Start Date and End Date both is required, Check again.';
		}
		//goto_datecheck_book();
		if(p_sdate == "" && p_edate == "" && hd_set == "" && lab_set == ""){
			e_error = 1;
