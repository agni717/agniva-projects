  	...
  	<div class="container">
		...
		<div class="table-responsive">
			<table id="d_table_show" class="table table-bordered table-striped">
				...
				<tbody>
				<?php foreach($user_list as $keys=>$users){ ?>
					<tr>
						
						<td><?php echo $keys+1; ?></td>
						<?php if($this->session->userdata('member_utype') == 1){ ?>
						<td>
						<a href="<?php echo base_url().'member/delete_user_from_portal/'.$users->fuser_id; ?>" class="btn btn-xs btn-danger">Delete User</a><br/>
						<a href="<?php echo base_url().'member/user_password_change/'.$users->fuser_id; ?>" class="btn btn-xs btn-warning" style="margin-top:5px;">Change Password</a>
						</td>
						<?php } ?>
						<td><?php if($users->fuser_type == 1){echo "Admin";}
						elseif($users->fuser_type == 2){echo "CMOH User";}
						elseif($users->fuser_type == 3){echo "BHHC User";}
						elseif($users->fuser_type == 4){echo "SDO User";}
						elseif($users->fuser_type == 5){echo "BDO User";}
						elseif($users->fuser_type == 6){echo "Viewer";} ?></td>
						<td><?php echo $users->fuser_name; ?></td>
						<td><?php echo $users->fuser_username; ?></td>
					</tr>
				<?php } ?>
				</tbody>
			</table>
